...
GUI.geometry("700x800+500+50") # ตั้งค่าขนาด GUI (500+50)ขนาดเเสดงผลตามขนาดจอ
...
def save(event=None): # ประกาศฟังก์ชัน, event=None เป็นตัวรับค่าจากการกด Enter ที่ Keyboard
	...
	try: 
		...
		stamp = datetime.now()
		dt = stamp.strftime('%Y-%m-%d %H:%M:%S') # กำหนดให้ใช้ลักษณะบ่งบอก วัน เดือน ปี เวลา ชั่วโมง นาที วินาที
...
header = ['Transaction_ID','Date-Time','Menu','Price','Item','Result']
...
headerwidth = [120,160,170,80,80,80] # กำหนดความกว้าง Header ขนาดตาราง
...
def updateCSV(): # ประกาศฟังก์ชันเพื่อเขียนทับ CSV ที่ทำการลบไปให้สร้างไฟล์ทับใหม่
	with open('Savedata.csv','w', newline='',encoding='utf-8') as f: # ให้เปิดใช้ csv ตัวนี้ขึ้นมา #with open มีป้องกันไว้เมื่อลืม close ไฟล์ csv
		fw = csv.writer(f)
		data = (alltransaction.values()) # เตรียมข้อมูลจาก alltransaction ให้กลายเป็น List 
		fw.writerows(data) # writerows มีเติม S ทำหลายบรรทัด # Multiple line from nested list [[],[],[]]
		print('Table was updated')
		


def DeleteRecord(event=None):
	check = messagebox.askyesno('Comfirm','Do you want to delete data?')
	print('YES/NO',check)

	if check == True:
		print('Delete')
		select = resulttable.selection() #ดึงข้อมูลจาก resulttable ดึง id มา
		#print(select)
		data = resulttable.item(select)
		data = data['values']
		transactionid = data[0]
		#print(transactionid)
		del alltransaction[str(transactionid)] # Delete Dara in dict ต้องแปลงเป็น Str
		updateCSV()
		update_table()
	else:
		print('Cancel')
...
resulttable.bind('<Delete>',DeleteRecord) # เวลากดปุ่ม Delete สามารถลบได้เลย ให้ใส่ (event=None) ตรง fn DeleteRecord
...
def update_table(): # สร้าง fn update ตาราง
	...
	try: # ใช้ try,except ตรวจสอบกรณีไม่พบ File CSV
		data = read_csv() # ดึง data csv

		for d in data:
			alltransaction[d[0]] = d #Creat Transaction Data
